...
class StartForm extends React.Component {
    ...
    incrementPosition(ev) {
        ...
        if (isNaN(this.state.noOfPlayers) || this.state.noOfPlayers === '') {
            ...
            if (
                noOfPlayers < this.props.minPlayers ||
                noOfPlayers > this.props.maxPlayers
            ) {
                this.setState({
                    warning: `Number of players must be between ${this.props.minPlayers} and ${this.props.maxPlayers}!`
                });
            } else {
                let playerNames = [];
        ...
        if (this.state.position === 1) {
            return (
                <form onSubmit={this.incrementPosition}>
                    ...
                    <input
                        type="text"
                        className="border-gray-400 border-2 rounded mt-3 mb-5"
                        onChange={ev => {
                    ...
                    <div className="flex justify-around">
                        <input
                            type="submit"
                            value="Continue"
                            className="bg-green-400 px-4 py-2 rounded font-light"
                        />
                        <button
                            className="bg-red-400 px-4 py-2 rounded font-light"
                            onClick={ev => {
            ...
            for (let i = 0; i < this.state.noOfPlayers; i++) {
                let inputField = (
                    <div key={i + ''}>
                        <span className="text-center text-xl font-semibold pr-3">
                            {'Player ' + (i + 1)}
                        </span>
                        <input
                            type="text"
