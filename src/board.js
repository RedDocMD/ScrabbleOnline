    ...
    for (let i = 0; i < size; i++) {
        for (let j = 0; j < size; j++) {
            ...
            if ((i === 0 || i === 14) && (j === 3 || j === 11))
                types[i][j] = 'double-letter';
            if ((j === 0 || j === 14) && (i === 3 || i === 11))
                types[i][j] = 'double-letter';
            if ((i === 1 || i === 13) && (j === 5 || j === 9))
                types[i][j] = 'triple-letter';
            if ((j === 1 || j === 13) && (i === 5 || i === 9))
                types[i][j] = 'triple-letter';
            if ((i === 2 || i === 12) && (j === 6 || j === 8))
                types[i][j] = 'double-letter';
            ...
            for (let j = 0; j < this.props.columns; j++) {
                row.push(
                    <td key={i + ' ' + j}>
                        <Cell
                            className={types[i][j]}
                            removeTileFromRack={this.props.removeTileFromRack}
                            row={i}
                            column={j}
                            content={this.props.cellContent[i][j]}
                            addTileToCell={this.props.addTileToCell}
                            removeTileFromCell={this.props.removeTileFromCell}
                        />
                    </td>
                );
            }
